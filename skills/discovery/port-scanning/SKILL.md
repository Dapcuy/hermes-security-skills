...
description: >
  ...
  via the endpoint_discovery capability, restricted to the connect scan method and
...
- Mengenumerasi port TCP pada host in-scope melalui capability `endpoint_discovery`, dengan metode connect scan (`-sT`) dan port range yang dibatasi persis oleh approval eksplisit (ROADMAP §8, §9, §13.1).
...
- `endpoint_discovery` — enumerasi port TCP dengan metode connect scan; wrapper image menegakkan policy bundle, budget, dan rate limit dari execution plan (ROADMAP §13.1).
...
3. Jalankan `endpoint_discovery` dengan metode connect scan; biarkan wrapper menegakkan rate dan budget; jangan mencoba mengakali parameter yang ditolak policy.
...
- Menjalankan `endpoint_discovery` dengan metode connect scan (`-sT`), terbatas pada host list dan port range yang tertulis di approval aktif.
...
- `http-traffic-analysis` — pembanding dari data traffic terekam saat scan tidak diperlukan.
