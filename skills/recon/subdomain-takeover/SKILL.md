...
description: >
  ...
  takeover indicators through httpx_probe probing and recorded history,
...
- `httpx_probe` — probe HTTP atas kandidat in-scope untuk mengambil status, title, dan isi permukaan respons yang dibutuhkan fingerprint vendor.
- `list_history` — membaca data terekam (respons lama, trafik sebelumnya) untuk membangun garis waktu perubahan subdomain tanpa mengirim traffic tambahan ke target.
...
3. Jalankan `httpx_probe` atas kandidat in-scope; tangkap status, title, dan isi permukaan respons kini.
...
- Menjalankan `httpx_probe` atas kandidat in-scope untuk fingerprint respons.
