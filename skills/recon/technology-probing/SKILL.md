...
description: >
  ...
  target, producing a technology map from the httpx_probe capability.
version: 0.1.0
risk: low
---
...
- Memprobe daftar URL/host in-scope secara aktif melalui capability `httpx_probe` untuk mengumpulkan status code, title halaman, dan indikasi teknologi per URL — satu permukaan metodologi HTTP GET yang ringan dan terukur.
...
- `httpx_probe` — probe HTTP atas daftar URL in-scope; menghasilkan status code, title, dan indikasi teknologi per URL.
...
3. Jalankan `httpx_probe` atas daftar yang sudah bersih; biarkan capability menegakkan rate dan budget.
...
- Menjalankan `httpx_probe` atas daftar URL yang lolos scope validation.
