...
description: >
  ...
  the subfinder_enum capability, without sending any request to the target.
version: 0.1.0
risk: low
---
...
- Menghasilkan daftar kandidat subdomain selengkap mungkin untuk domain yang in-scope, melalui capability `subfinder_enum` yang mengumpulkan data dari sumber pihak ketiga yang publik — tanpa mengirim satu request pun ke infrastruktur target.
...
- `subfinder_enum` — agregasi subdomain pasif dari third-party data sources; egress capability tertuju ke sumber data publik, bukan ke target.
...
2. Jalankan `subfinder_enum` atas domain tersebut; biarkan capability mengelola kueri ke data source sesuai budget dan rate limit execution plan.
...
- Menjalankan `subfinder_enum` atas domain yang terdaftar di scope entries.
