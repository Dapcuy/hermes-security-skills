...
// Index adalah inverted index sederhana in-memory (§61 full-text retrieval):
// token → (entryID → frekuensi kemunculan). MVP sesuai ROADMAP §49
...
// BuildIndex membangun index atas semua entry di store. Sumber token:
...
func (ix *Index) Search(query string) ([]Scored, error) {
	...
	if len(tokens) == 0 {
		return nil, fmt.Errorf("knowledge: query kosong")
	}
